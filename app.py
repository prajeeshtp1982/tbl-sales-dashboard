...
zip_files = list(current_folder.glob("*.zip"))

st.sidebar.header("📁 Data Source")

uploaded_file = st.sidebar.file_uploader(
    "Upload Sales ZIP file",
    type=["zip"]
)
...
if uploaded_file is not None:
    temp_path = current_folder / "uploaded_sales.zip"
    with open(temp_path, "wb") as f:
        f.write(uploaded_file.getbuffer())
    default_zip = temp_path
elif zip_files:
    default_zip = zip_files[0]

if default_zip is None:
    st.warning("Please keep your Sales ZIP file in the same folder or upload it from sidebar.")
    st.stop()

st.sidebar.success(f"Loaded: {default_zip.name}")

df = load_zip_file(default_zip)

if df.empty:
    st.error("No usable sales data found in the ZIP file.")
    st.stop()
...
exclude_item_words = (
    ...
    "CARRIAGE|FREIGHT|FREIGHT CHARGES|CUSTOM DUTY|"
    "ADMIN FEES|PROCESSING FEES|SHIPPING CHARGES|"
...
# Remove service/non-product rows that may appear as customers/items when brand is blank
service_words = (
    "SALES|CARRIAGE|FREIGHT|FREIGHT CHARGES|CUSTOM DUTY|"
    "ADMIN FEES|PROCESSING FEES|SHIPPING CHARGES|"
    "SHIPPING|DELIVERY CHARGES|CHARGES|FEES|DUTY"
)

df = df[
    ~(
        (df["Brand"].fillna("").str.strip() == "")
        & (
            df["Item Name"]
            .str.upper()
            .str.contains(service_words, na=False, regex=True)
        )
    )
]
...
with tab5:
    ...
    if comparison_type == "Customer vs Customer":
        ...
        fig_customer_brand.update_xaxes(type="category")
